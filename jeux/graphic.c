...
void init_textures(world_t *world,SDL_Renderer * renderer)
{
	...
	world->ennemy.image = charger_image_transparente("sprite.bmp", renderer,r,g,b);
}
...
void refresh_graphic(world_t *world,SDL_Renderer * renderer)
{
	SDL_RenderClear(renderer);
		...
		if(world->ennemy.est_visible == 1) 
			SDL_RenderCopy(renderer,world->ennemy.image, &(world->ennemy.SrcR_sprite), &(world->ennemy.DestR_sprite) );
...
void clear_textures(world_t *world)
{
	...
	SDL_DestroyTexture(world->ennemy.image);
